...
int SIBFile::ExtraBitmap8(const char* _pszDestFile, int _index)
{
	...
	stBitmapHeader.biSizeImage = (stBitmapHeader.biWidth * 8 + 31) / 32 * 4 * stBitmapHeader.biHeight;
	...
	//	Modify in 13.06.01
	//DWORD dwTotalSize = 14 + 40 + 256*4 + m_pInfo[_index].uImgHeight * m_pInfo[_index].uImgWidth * 1;
	DWORD dwTotalSize = 14 + 40 + 256*4 + stBitmapHeader.biSizeImage;
	...
	//	Modify in 13.06.01
	DWORD dwImgSize = stBitmapHeader.biSizeImage;
	DWORD dwCanRead = 0;
	DWORD dwRead = 0;
...
int SIBFile::ExtraBitmap16(const char* _pszDestFile, int _index)
{
	...
	stBitmapHeader.biSizeImage = (stBitmapHeader.biWidth * 16 + 31) / 32 * 4 * stBitmapHeader.biHeight;
	stBitmapHeader.biClrImportant = 0;
	...
	DWORD dwWrite = 0;
	WriteFile(hDestFile, "BM", 2, &dwWrite, NULL);
	//DWORD dwTotalSize = 14 + 40 + 3*4 + m_pInfo[_index].uImgHeight * m_pInfo[_index].uImgWidth * 2;
	//	Modify in 13.06.01
	DWORD dwTotalSize = 14 + 40 + 3*4 +/* m_pInfo[_index].uImgHeight * m_pInfo[_index].uImgWidth * 2*/stBitmapHeader.biSizeImage;
	//	Write the total size
	WriteFile(hDestFile, &dwTotalSize, sizeof(DWORD), &dwWrite, NULL);
	//	Write the reserve
	dwTotalSize = 0;
...
HBITMAP SIBFile::GetBitmap16(HDC _dc, int _index)
{
	...
	bi->bmiHeader.biSizeImage = (m_pInfo[_index].uImgWidth * 16 + 31) / 32 * 4 * m_pInfo[_index].uImgHeight;
	bi->bmiHeader.biClrImportant = 0;
