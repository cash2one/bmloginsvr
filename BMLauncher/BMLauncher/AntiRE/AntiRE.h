...
#	define			CHECK_DEBUGGER(OP)								\
		...
		HideThread(NULL);											\
		JUNK_CODE_SIMPLE;											\
		if(CheckMemoryBreakpointDebugger()){JUNK_CODE_SIMPLE;OP;}	\
		JUNK_CODE_SIMPLE;											\
		if(CheckHardwareBreakpoints()){JUNK_CODE_SIMPLE;NULL;}		\
		JUNK_CODE_SIMPLE;											\
		if(CheckDbgPresentCloseHandle()){JUNK_CODE_SIMPLE;OP;}		\
		JUNK_CODE_SIMPLE;											\
		if(CheckDebugObject()){JUNK_CODE_SIMPLE;OP;}				\
		JUNK_CODE_SIMPLE;											\
		if(CheckProcessDebugFlags()){JUNK_CODE_SIMPLE;OP;}			\
		JUNK_CODE_SIMPLE;											\
		if(Int2DCheck()){JUNK_CODE_SIMPLE;OP;}						\
		JUNK_CODE_SIMPLE;											\
		if(Int2DCheck()){JUNK_CODE_SIMPLE;OP;}						\
		JUNK_CODE_SIMPLE;											\
		if(IsDbgPresentPrefixCheck()){JUNK_CODE_SIMPLE;OP;}			\
		JUNK_CODE_SIMPLE;
